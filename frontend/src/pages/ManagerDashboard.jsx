...
const ManagerDashboard = () => {
  ...
  const filteredTasks = tasks.filter((task) => {
    return (
      (!filter.employee ||
        task.username.toLowerCase().includes(filter.employee.toLowerCase())) &&
      (!filter.tags ||
        task.tags.some((tag) =>
  ...
  const updateTask = async () => {
    try {
      ...
      const response = await fetch(
        `http://localhost:8000/api/tasks/${viewTask.id}/`,
        {
          method: "PATCH",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      updatedTask.tags =
        typeof updatedTask.tags === "string"
    ...
    datasets: [
      {
        ...
        data: [
          filteredTasks
            ...
            .reduce((sum, task) => sum + task.hours_spent, 0),
          filteredTasks
  ...
  const handleEmployeeSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        Swal.fire(
          "Error",
          errorData.message || "Failed to register employee",
          "error"
        );
      }
    } catch (error) {
  ...
  const mostUsedTag = allTags.sort(
    (a, b) =>
      allTags.filter((tag) => tag === b).length -
        ...
        <div className="container mx-auto p-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            <input
              ...
              onChange={(e) =>
                setFilter({ ...filter, employee: e.target.value })
              }
            />
            <input
              type="text"
                  ...
                  {filteredTasks.map((task, idx) => (
                    ...
                    >
                      ...
                      <td className="p-3 flex items-center gap-2">
                        ...
                        {/* Conditionally render buttons or text based on the task status */}
            ...
            <div className="fixed inset-0 bg-black bg-opacity-40 flex justify-center items-center z-50">
              <div className="bg-white p-6 rounded-2xl w-full max-w-lg shadow-lg">
                ...
                <div className="space-y-3 text-sm">
                  <p>
                    <strong>Employee:</strong> {viewTask.employee}
                  </p>
                  <p>
                  ...
                  <p>
                    <strong>Hours:</strong> {viewTask.hours_spent}
                  </p>
                  <p>
                    <strong>Tags:</strong>{" "}
                    {(Array.isArray(viewTask.tags) ? viewTask.tags : []).join(
                      ", "
                    )}
                  </p>
                  <p>
                    <strong>Date:</strong> {viewTask.date}
                  </p>
